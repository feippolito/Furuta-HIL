...
#
#  $RCSfile: Furuta_Model.mk $ $Revision: 2.87 $ $Date: 2014/08/29 10:59:58MESZ $
...
MAKEFILE                       = Furuta_Model.mk
BUILDARGS                      = SKIP_TASK_CONFIGURATION=on EXTMODE_STATIC_ALLOC=0 TMW_EXTMODE_TESTING=0 EXTMODE_STATIC_ALLOC_SIZE=1000000 EXTMODE_TRANSPORT=0 MAT_FILE=0 NCLIENT="OPTION_DISABLED" ISPROTECTINGMODEL=NOTPROTECTING
...
COMPUTER                       = PCWIN64
MODEL                          = Furuta_Model
MODULES                        = Furuta_Model_data.c Furuta_Model_trc_ptr.c rtGetInf.c rtGetNaN.c rt_nonfinite.c rt_zcfcn.c 
S_FUNCTIONS                    = 
S_FUNCTIONS_LIB                = 
SOLVER                         = 
NUMST                          = 2
...
%if !defined(_IS_MP_BUILD)
# For RTI-SP: Platform option for loader.
% set PLATFORM_OPT  = -p $(PLATFORM)
%endif   # NOT _IS_MP_BUILD
...
%  else
%    set SDFFILE = $(WORKDIR)$(MP_APPL_NAME).sdf
%  endif
%else
...
RTWCUST_INCLUDES = \
	"$(START_DIR)\Furuta_Model_rti1006" \
	"$(START_DIR)"
...
start_check :
	...
	%if !(exists("$(CC,"=)") && exists("$(AS,"=)") && exists("$(LD,"=)") && exists("$(AR,"=)"))
	  %echo ERROR: Cannot find x86 compiler. Possible reasons are:
	...
	%if !null($(USER_MAKEFILE_VERSION))
	  ...
	  %endif
	%endif
	%if (!defined(_IS_MP_BUILD)) && defined(PLATFORM)
	  %if "$(PLATFORM)" == ""
...
postlink_update_uuid:
    ...
    %if exists($(SDFFILE)) && exists($(PROGRAM))
        ...
        $(EXEC_SHELL) $(SG) $(PROGRAM) /n /q 1 /upo /sdf $(SDFFILE) /rtp $(MP_CPU_NAME)
        %if $(status)
          %set _STATUS = $(status)
